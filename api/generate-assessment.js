...
- ADVANCED (introductory NCEA Level 3 concepts — first-exposure, NOT full Year 13 mastery): Basic spectroscopy at recognition level (e.g. IR: identifying a carbonyl peak at ~1700 cm⁻¹ or O-H broad stretch; mass spec: reading molecular ion peak to find molecular mass; do NOT ask full NMR interpretation); introductory kinetics (rate = k[A]^m concept; what doubling [A] does to rate given a rate law; do NOT ask Arrhenius equation calculations); radioactivity basics (alpha/beta/gamma properties; half-life concept calculation). Questions must be accessible to a talented Year 12 student who has read ahead — not requiring Year 13 coursework completion.`
    }
    if (nzcLevel.includes('Level 8') || nzcLevel.includes('NCEA Level 3') || nzcLevel.includes('Year 13')) {
...
- ADVANCED (NCEA Level 3 skills): Geographic research methodology; complex spatial analysis; resource management debates; political geography`
    ...
    if (nzcLevel.includes('Level 8') || nzcLevel.includes('NCEA Level 3') || nzcLevel.includes('Year 13')) {
      return `NCEA GEOGRAPHY LEVEL 3 (Year 13) CALIBRATION:
- FOUNDATION (NCEA Level 2 skills): Urban structure models; glacial/fluvial geomorphology; climate change causes and consequences; global development gap and interventions; geographic research methods
- DEVELOPING (NCEA Level 3 skills): Geographic concepts applied at an advanced level (interconnection at a global scale — how political, economic, and environmental systems interact; sustainability — evaluating trade-offs between development and environmental protection; scale — local to global feedback loops); resource management (types of resources — renewable/non-renewable/flow; tragedy of the commons; common pool resource management; sustainable resource use — fisheries, water, forests; Integrated Catchment Management; NZ resource management under the RMA; case studies of successful and failed resource management); geographic research methodology in depth (research design — identifying a geographic question, justifying methods; primary data collection in the field — transects, counts, questionnaires, interviews, mapping; secondary data analysis — census, satellite imagery, government statistics; GIS analysis; ethical considerations in geographic research; presenting findings — maps, graphs, statistical analysis including Spearman rank correlation); political geography (nation-state vs stateless peoples; sovereignty disputes; geopolitics of energy and water; international environmental agreements — Paris Agreement, UNCLOS; role of international organisations in managing global commons)
- ADVANCED (university/scholarship skills): Spatial econometrics; political ecology; post-colonial geographic theory; geographic information science (GIS programming)`
    }
...
- ADVANCED (NCEA Level 3 skills): Cloud computing; microservices architecture; advanced machine learning; research into emerging technologies`
    ...
    if (nzcLevel.includes('Level 8') || nzcLevel.includes('NCEA Level 3') || nzcLevel.includes('Year 13')) {
      return `NCEA DIGITAL TECHNOLOGY LEVEL 3 (Year 13) CALIBRATION:
- FOUNDATION (NCEA Level 2 skills): Agile/waterfall SDLC; Git version control; RESTful APIs; JWT authentication; subnetting/CIDR; AI concepts (supervised/unsupervised learning); digital forensics basics
- DEVELOPING (NCEA Level 3 skills): Cloud computing (IaaS/PaaS/SaaS models; AWS/Azure/GCP services — EC2, S3, Lambda; auto-scaling; load balancing; serverless architecture; cost models; shared responsibility model for security); microservices and system design (monolith vs microservices trade-offs; service-oriented architecture; message queues — Kafka, RabbitMQ; API gateway; containerisation with Docker; container orchestration with Kubernetes; CAP theorem; eventual consistency; distributed systems challenges — fault tolerance, partitioning); advanced security (penetration testing methodology — reconnaissance, scanning, exploitation, reporting; OWASP Top 10 vulnerabilities in depth; threat modelling; zero-trust architecture; PKI and certificate management; cryptographic protocols — TLS 1.3, RSA, AES; incident response planning; GDPR and NZ Privacy Act compliance); emerging technologies and research (evaluating new technologies — AR/VR, blockchain, quantum computing, edge computing; conducting a structured literature review; ethical implications of emerging tech — bias in AI, digital divide, surveillance capitalism; writing a technical research report)
- ADVANCED (university/scholarship skills): Formal verification of algorithms; type theory; advanced cryptography (elliptic curve, zero-knowledge proofs); distributed consensus algorithms (Raft, Paxos)`
    }
...
- ADVANCED (NCEA Level 1 skills): Advanced harmony; modulation; composing in a specific style with analytical commentary; music history in depth (specific periods and composers)`
    ...
    if (nzcLevel.includes('Level 6') || nzcLevel.includes('NCEA Level 1') || nzcLevel.includes('Year 11')) {
      return `NCEA MUSIC LEVEL 1 (Year 11) CALIBRATION:
- FOUNDATION (Year 7-8 skills): Modes; 7th chords; chord progressions; musical texture; compositional devices (canon, augmentation, inversion); 20th century styles
- DEVELOPING (core NCEA Level 1 skills — accessible to a well-prepared Year 11 student): Diatonic harmony (chord functions I, ii, iii, IV, V, vi, vii°; cadences — perfect, imperfect, plagal, interrupted; modulation to the dominant or relative major/minor at a basic level); music history and context at a broad level (Baroque — basso continuo, imitative counterpoint; Classical — sonata form introduction, balanced phrase structure, homophony; Romantic — programme music concept, nationalism; 20th century — impressionism, serialism, minimalism in overview; jazz — blues scale, 12-bar blues form); composition skills (writing a short melody using a given chord progression; identifying and using sequence and repetition; basic part-writing with two voices); aural skills (identifying intervals — perfect 4th/5th, major/minor 3rd/6th, octave; recognising major, minor, diminished, dominant 7th chord types; clapping back a 4-bar rhythm). NOTE: keep developing questions at a level where a well-prepared Year 11 NCEA student would answer correctly — these should NOT require secondary dominants or non-chord tone analysis.
- ADVANCED (NCEA Level 2 skills): Secondary dominants (V/V, V/IV usage and resolution); non-chord tones (passing note, suspension, appoggiatura, pedal point — identify and use in writing); chromatic modulation; composing with complex formal structures (ABA, rondo, through-composed); music analysis using detailed music theory vocabulary; advanced aural skills (transcribing a short passage including chromatic notes)`
    }
    if (nzcLevel.includes('Level 7') || nzcLevel.includes('NCEA Level 2') || nzcLevel.includes('Year 12')) {
      return `NCEA MUSIC LEVEL 2 (Year 12) CALIBRATION:
- FOUNDATION (NCEA Level 1 skills): Modulation; secondary dominants; voice-leading; Baroque/Classical/Romantic/20th century context; compositional techniques; aural transcription
- DEVELOPING (NCEA Level 2 skills): Chromatic harmony (borrowed chords — modal mixture; augmented 6th chords — Italian, French, German; Neapolitan chord; chromatic modulation techniques); advanced counterpoint (two-voice species counterpoint; invertible counterpoint; fugal writing — exposition, episode, stretto); large-scale formal analysis (sonata form in depth — exposition/development/recapitulation; rondo; variation sets; through-composed forms; Romantic symphony structure); 20th century compositional techniques (twelve-tone serialism — row construction and operations; polytonality; extended techniques; minimalism — phase shifting, additive process; spectralism); music technology (digital audio workstations — recording, editing, mixing, mastering; synthesis types — subtractive, FM, granular, wavetable; MIDI programming; music production in popular genres)
- ADVANCED (NCEA Level 3 skills): Post-tonal analysis; advanced orchestration; electroacoustic composition; research into a composer or style`
    }
    if (nzcLevel.includes('Level 8') || nzcLevel.includes('NCEA Level 3') || nzcLevel.includes('Year 13')) {
